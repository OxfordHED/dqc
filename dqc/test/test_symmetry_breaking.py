...
def test_broken_symmetry_lower_at_dissociation():
    ...
    qc = KS(_h2(5.0), xc=XC, restricted=False).run(break_symmetry=0.785, diagnose=True)
...
def test_broken_symmetry_collapses_near_equilibrium():
    ...
    qc = KS(_h2(1.4), xc=XC, restricted=False).run(break_symmetry=0.785, diagnose=True)
    assert qc.is_converged()
...
def test_broken_symmetry_gradient_matches_finite_difference():
    def energy(a):
        ...
        qc = KS(_h2(5.0), xc=xc, restricted=False).run(
            break_symmetry=0.785, diagnose=True, conv_tol=1e-6)
        assert qc.is_converged()
        return qc.energy(), xc
